function [hCMap,hDose,hCt,hContour,hIsoDose] = matRad_plotSliceWrapper(axesHandle,ct,cst,cubeIdx,dose,plane,slice,windowXY,thresh,alpha,contourColorMap,...
    doseColorMap,doseWindow,doseIsoLevels,voiSelection,suppressCbar,colorBarLabel,boolPlotLegend,varargin)
...
%   suppressCbar    do not generate colorbar (useful when usage in subplots)
...
if ~exist('windowXY','var') || isempty(windowXY)
    windowXY = [];
end
...
if ~exist('suppressCbar','var') || isempty(suppressCbar)
    suppressCbar = false;
end
