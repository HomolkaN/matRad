function IDD = matRad_calcIDD_pencil(doseCube,calcProfile,direction,peakPos)
...
if nargin < 4
    [~, linearIndex] = max(doseCube(:));
    [peakPos(1), peakPos(2), peakPos(3)] = ind2sub(size(doseCube), linearIndex);
end
...
switch direction
    case 'y'
        if ~calcProfile
            ...
            IDD = doseCube(:,peakPos(2),peakPos(3));
        end
    case 'x'
        if ~calcProfile
            IDD = sum(sum(doseCube,1),3);
        else
            IDD = doseCube(peakPos(1),:,peakPos(3));    
        end
    ...
    case 'z'
        if ~calcProfile
            ...
            IDD = doseCube(peakPos(1),peakPos(2),:);    
        end
